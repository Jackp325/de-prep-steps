...
def get_char_code(string):
    return f'The ASCII code for {string} is {ord(string)}'
...
def check_battery_level(num):
    if num <= 5:
        return f"Warning - battery level low: {num}%, please charge your device"
    if num <= 99:
        return f"Battery level: {num}%"
