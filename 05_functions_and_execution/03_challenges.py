...
def create_profile_description(person):
    ...
    likes = "like" if person.get("likes_to_code") else "don't like"
    return f"My name is {name} and I {likes} to code."
...
def read_traffic_light(colour):
    ...
    return responses.get(colour.lower(), "Invalid colour")
...
def update_coin_machine(coin_machine, coin):
    coin_machine[coin] = coin_machine.get(coin, 0) + 1
...
def update_position(coords, direction):
    move = {
        "right": [1, 0],
        "left": [-1, 0],
        "up": [0, 1],
    ...
    return [c + m for c, m in zip(coords, move[direction])]
...
def check_game(dice_roll, coin_toss):
    return dice_roll >=3 and coin_toss == "H"
...
def add_coins(coin_collection, coin):
    ...
    coin_collection[position[coin]].append(coin)
