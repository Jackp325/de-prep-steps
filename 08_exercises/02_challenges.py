...
def is_valid_mobile_number(mobile_number):
    ...
    return any(
        mobile_number.startswith(prefix)
        and len(mobile_number) == length
        and mobile_number[digit_start:].isdigit()
        for prefix, length, digit_start in valid_formats
    )
...
def get_williams(names):
    return [name for name in names if name.endswith('Williams')]
...
def generate_matrix(number):
    matrix = []
    [matrix.append([None for _ in range(number)]) for _ in range(number)]
    return matrix
...
def find_wrong_way_fruit(orchard):
    for i, fruit in enumerate(orchard):
...
def dna_pairs(dna_string):
    ...
    return [
        c + pair_dict[c] 
        for c in map(str.upper, dna_string) 
        if c in pair_dict
        ]
