...
def tally_people_in_manchester(people):
    return sum(1 for p in people if p['lives']['city'] == 'Manchester')
...
def pluralise_keys(dictionary):
    ...
    for key, value in dictionary.items():
        if isinstance(value, list) and not key.endswith("s"):
            result[key + "s"] = value
        else:
            result[key] = value
...
def replace_letters_with_x(string):
    return ''.join('X' if c.isalpha() else c for c in string)
